...
loadDatabase()
/*
const { state, saveCreds } = await useMultiFileAuthState(path.resolve('./mari-sessions'))
...
version: [2, 2413, 1],
getMessage: async (key) => (store.loadMessage(key.remoteJid, key.id) || store.loadMessage(key.id) || {}).message, // get message disamping untuk mengatasi pesan gagal dikirim, "menunggu pesan", dapat dicoba lagi
...
/*
const { state, saveCreds } = await useMultiFileAuthState('mari-sessions');
const msgRetryCounterCache = new NodeCache() // for retry message, "waiting message"
const connectionOptions = {
printQRInTerminal: true,
logger: pino({ level: 'fatal' }),
auth: state,
browser: [`Mari Bot WhatsApp`, "Safari", "1.0.0"],
        getMessage: async key => {
            return {
                // Add your message content here if needed
                conversation: "Hi Im Mari Bot"
            }
        },
       msgRetryCounterCache, // Resolve waiting messages
    defaultQueryTimeoutMs: undefined, // for this issues https://github.com/WhiskeySockets/Baileys/issues/276
...
// Thanks to Danil and ImYanXiao for this code
...
store?.readFromFile('./mari-store.json')
setInterval(() => { // save every 10s
store?.writeToFile('./mari-store.json')
}, 10_000)

// Multi auth
const { version, isLatest } = await fetchLatestBaileysVersion()
let { state, saveCreds } = await useMultiFileAuthState(path.resolve('./ᴍʏʙᴏᴛᴢ'))
const msgRetryCounterCache = new NodeCache() // for retry message, "waiting message"
...
conn.sendMessage(`6289691512285@s.whatsapp.net`, { text: `Bot Sukses Tersambung` });
}
}
