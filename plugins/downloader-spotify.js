...
const handler = async (m, { conn, text, usedPrefix, command}) => {
    if (!text) throw `Usage example ${usedPrefix + command} Spiral by longman`;
    await conn.reply(m.chat, 'Currently searching, please wait...', m);
    
    const result = await searchSpotify(text);
    let responseText = `*乂 Spotify Search*\n\n`;
    ...
    responseText += `_Please reply this message with numbers [e.g.: 1 or 2] to get the song link_\n`;
    responseText += `⏰ *Timeout :* 60 Second!`;
...
handler.before = async (m, { conn }) => {
    ...
    if (inputNumber >= 1 && inputNumber <= result.length) {
        ...
        try {
            ...
            const captionvid = `*乂 Spotify Play*\n\n∘ *Title :* ${selectedTrack.name}\n∘ *Artist :* ${selectedTrack.artists}\n∘ *Duration :* ${durationText}\n∘ *Link :* ${selectedTrack.link}\n∘ *Audio Downloaded in :* ${timeTaken}\n\n*Note :*\nIf the music doesn't appear, aka an error, you can use this command *.spotifydl ${selectedTrack.link}*`;
            const title = '';

            let sayangloli = await conn.sendMessage(m.chat, { text: captionvid,
                contextInfo: {
                    externalAdReply: {
                        title: `${selectedTrack.name}`,
                        body: `${selectedTrack.artists}`,
            ...
            await conn.sendMessage(m.chat, { audio: { url: audioLink }, mimetype: 'audio/mpeg', 
                contextInfo: {
                    externalAdReply: {
                        title: `${selectedTrack.name}`,
                        body: `${selectedTrack.artists}`,
